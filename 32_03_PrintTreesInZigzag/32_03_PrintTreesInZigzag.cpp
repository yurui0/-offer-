// 32_03_PrintTreesInZigzag.cpp : 定义控制台应用程序的入口点。
//
...
void Print(BinaryTreeNode* pRoot)
{
	...
	std::stack<BinaryTreeNode*> levels[2];//这里是定义的两个栈，level[0]是保存奇数层的需要打印的节点,lecel[1]是保存偶数层的
	int current = 0;
	int next = 1;

	levels[current].push(pRoot);
	while (!levels[0].empty() || !levels[1].empty())//两个栈都没空，即存在需要打印的节点
	{
		BinaryTreeNode* pNode = levels[current].top();//当前层开始打印，一开始是从第一层开始，因此current=0；
		levels[current].pop();

		printf("%d ", pNode->m_nValue);

		if (current == 0)//current==0表示当前层是奇数层，接下来是将当前节点的子节点加到另一个栈中
		{
