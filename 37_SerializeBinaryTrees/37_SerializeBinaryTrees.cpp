// 37_SerializeBinaryTrees.cpp : 定义控制台应用程序的入口点。
//
// 面试题37：序列化二叉树
...
{
	if (pRoot == nullptr)//如果该节点是空节点，则将其序列化为$，（后面的,是为了区分）
	{
		stream << "$,";
		return;
	}
	//如果节点处有数字，则保存值，然后接着序列化其左右子节点
	stream << pRoot->m_nValue << ',';
	Serialize(pRoot->m_pLeft, stream);
	Serialize(pRoot->m_pRight, stream);
}

bool ReadStream(istream& stream, int* number)//每次从数据流中读取一个数字或者一个字符“$”,如果读出数字返回true，否则返回false;
{
	if (stream.eof())
		return false;
	...
	if (i > 0 && buffer[0] != '$')
	{
		*number = atoi(buffer);//atoi把字符串转换为整型数
		isNumeric = true;
	}
...
{
	int number;
	if (ReadStream(stream, &number))//如果读入的是数字，则遍历其左右两个子节点
	{
