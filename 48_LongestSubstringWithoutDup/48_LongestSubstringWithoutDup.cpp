// 48_LongestSubstringWithoutDup.cpp : 定义控制台应用程序的入口点。
//
// 面试题48：最长不含重复字符的子字符串
...
{
	int curLength = 0;//这个用来表示不包含重复字符的子字符串的长度 
	int maxLength = 0;//这个用来表示不包含重复字符的子字符串的最长长度

	int* position = new int[26];//一个长度为26的数组position用来存储每个字符上次出现在字符串中的位置的下标
	for (int i = 0; i < 26; ++i)
		position[i] = -1;

	for (int i = 0; i < str.length(); ++i)//从字符串头开始循环，依次寻找
	{
		int prevIndex = position[str[i] - 'a'];//这里是计算当前i位置处的字符对应a-z这26个字符的相对位置（0-26）
		if (prevIndex < 0 || i - prevIndex > curLength)//如果一个字符没有出现、或者出现的间隔要比不包含重复字符的子字符串长度长，则不会对其产生应该
			++curLength;
		else
		{
		...
		position[str[i] - 'a'] = i;//更改当前字符出现在字符串中的位置，赋值为i
	}
