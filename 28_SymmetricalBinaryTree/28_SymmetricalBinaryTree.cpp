// 28_SymmetricalBinaryTree.cpp : 定义控制台应用程序的入口点。
//
// 面试题28：对称的二叉树
...
bool isSymmetrical(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2)//比较两个节点的值是否相等
{
	if (pRoot1 == nullptr && pRoot2 == nullptr)//此时说明比较完成，是对称的，理由是对称比较的过程中考虑了nullptr的比较，这两个是在一行的左右两侧
		return true;

	if (pRoot1 == nullptr || pRoot2 == nullptr)//在一直匹配到最后的过程中，没有匹配完就发现有不对称的现象
		return false;

	if (pRoot1->m_nValue != pRoot2->m_nValue)//对称节点值不一样，肯定不对称
		return false;

	return isSymmetrical(pRoot1->m_pLeft, pRoot2->m_pRight)  //判断前序遍历和对称的前序遍历是否一致
